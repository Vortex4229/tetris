...
clear_text = box_font.render("No lines cleared yet", True, white)
name_text = box_font.render("Made by Paulo Bello", True, white)
version_text = box_font.render("v1.2", True, white)
...
class Piece:
    ...
    def dropYCalculator(self):
        ...
        self.squares_to_bottom = min(square_count_list)

        for block in self.block_list:
            if grid[block.position[1] + self.squares_to_bottom][block.position[0]] == 1:
                self.squares_to_bottom -= 1

        for block in self.block_list:
            self.drop_y.append(block.position[1] + self.squares_to_bottom)

    def draw(self):
        if self.active:
            for block in self.block_list:
                block.draw()
            for x in range(len(self.drop_y)):
                pygame.draw.rect(window, self.highlight_color, (100 + self.block_list[x].position[0] * 30, 100 + self.drop_y[x] * 30, 30, 30), 2)
            self.piece_frame += 1
        else:
            for block in self.block_list:
                block.draw()
...
def updateScore(type, lines_cleared=0, blocks_dropped=0):
    global score
    global level
    global total_lines
    global score_text2
    global clear_text
    global clear_text_toggle
    ...
    if type == "line_clear":
        ...
        match lines_cleared:
            case 1:
                score += 40 * (level + 1)
                clear_text = box_font.render(f"Single! +{40 * (level + 1)} points!", True, white)
                clear_text_toggle = True
            case 2:
                score += 100 * (level + 1)
                clear_text = box_font.render(f"Double! +{100 * (level + 1)} points!", True, white)
                clear_text_toggle = True
            case 3:
                score += 300 * (level + 1)
                clear_text = box_font.render(f"Triple! +{300 * (level + 1)} points!", True, white)
                clear_text_toggle = True
            case 4:
                score += 1200 * (level + 1)
                clear_text = box_font.render(f"Tetris! +{1200 * (level + 1)} points!", True, white)
                clear_text_toggle = True
...
def hardDrop():
    ...
    for x in range(len(current_piece.block_list)):
        current_piece.block_list[x].position[1] = current_piece.drop_y[x]

    updateScore("hard_drop", blocks_dropped=current_piece.squares_to_bottom)
    pieceSwitch()


def removeBlocks(past_blocks, row):
    copy_blocks = []
...
def lineClearCheck():
    ...
    for i in range(len(to_remove)):
        past_blocks = removeBlocks(past_blocks, to_remove[i] + i)
        for block in past_blocks:
            if (block.position[1] < to_remove[i] + i):
                block.position[1] += 1

    if lines_filled > 0:
        updateScore("line_clear", lines_cleared=lines_filled)
...
while True:
    ...
    if not menu and not pause_menu and not game_over:
        ...
        if fast_drop:
            if current_piece.piece_frame % int(speed / 2) == 0 and current_piece.piece_frame != 0:
                current_piece.move()
        ...
        if clear_text_toggle == True:
            if current_piece.piece_frame == 120:
                clear_text_toggle = False
    ...
    if menu:
        ...
        if clear_text_toggle == True:
            window.blit(clear_text, (430, 668))
